...
import reportCollectionsFakerFactory from "./reportCollectionsFakerFactory";
...
const ReportCollectionsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = reportCollectionsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("reportCollections").create(elem));
...
export default connect(mapState, mapDispatch)(ReportCollectionsFakerDialogComponent);
